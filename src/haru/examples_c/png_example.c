/*
 * << H a r u -- Free PDF Library >> -- png_example.c
 *
        ...
        float x, float y, char *text)
{
    ...
    strcpy(filename1, "../examples/pngsuite");
    strcat(filename1, FILE_SEPARATOR);
    strcat(filename1, filename);

    image = pdf_png_image_new(doc);
    CHECK_ERROR2(image);

    CHECK_ERROR(pdf_png_image_load_from_file(image, filename1));
    
    CHECK_ERROR(pdf_doc_add_xobject(doc, image, NULL));

    CHECK_ERROR(pdf_stream_add_filter(image, PDF_FILTER_DEFLATE));
...
int main()
{
    ...
    ret = draw_image(doc, canvas, "basn0g01.png", 100, 
            pdf_contents_get_height(canvas) - 150, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn0g02.png", 200, 
            pdf_contents_get_height(canvas) - 150, "2bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn0g04.png", 300, 
            pdf_contents_get_height(canvas) - 150, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn0g08.png", 400, 
            pdf_contents_get_height(canvas) - 150, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn2c08.png", 100, 
            pdf_contents_get_height(canvas) - 250, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn2c16.png", 200, 
            pdf_contents_get_height(canvas) - 250, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn3p01.png", 100, 
            pdf_contents_get_height(canvas) - 350, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn3p02.png", 200, 
            pdf_contents_get_height(canvas) - 350, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn3p04.png", 300, 
            pdf_contents_get_height(canvas) - 350, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn3p08.png", 400, 
            pdf_contents_get_height(canvas) - 350, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn4a08.png", 100, 
            pdf_contents_get_height(canvas) - 450, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn4a16.png", 200, 
            pdf_contents_get_height(canvas) - 450, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn6a08.png", 100, 
            pdf_contents_get_height(canvas) - 550, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    ret = draw_image(doc, canvas, "basn6a16.png", 200, 
            pdf_contents_get_height(canvas) - 550, "1bit grayscale.");
    if (ret != PDF_SUCCESS)
        return -1;

    CHECK_ERROR(pdf_doc_write_to_file(doc, "png_example.pdf"));
