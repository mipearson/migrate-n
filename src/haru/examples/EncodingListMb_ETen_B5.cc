...
int main()
{
    ...
    try {
        ...
        PdfCMap* map1 = new PdfCMap_ETen_B5_H();
        doc->RegisterObject(map1);
        doc->AddType0Font(new PdfMingFontDef(), "ming", map1);
        ...
        for (unsigned int a = 0xa1; a < 0xfa; a++) 
            DrawPage(doc, outline_item, a);
        
        /* Save the document to the file. */
        doc->WriteToFile("EncodingListMb_ETen_B5.pdf");
...
void DrawPage(PdfDoc *doc, PdfOutlineItem *outline_item, unsigned int from)
{
    ...
    PdfFont* f = doc->FontMgr()->GetFont("ming");
