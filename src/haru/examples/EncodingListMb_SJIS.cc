...
/*
 * Character lists for Japanese SJIS encoding.  
 */
...
int main()
{
    ...
    try {
        ...
        PdfCMap* map1 = new PdfCMap_90ms_RKSJ_H();
        doc->RegisterObject(map1);
        doc->AddType0Font(new PdfGothicFontDef(), "gothic", map1);
        
        /* Create outline root */
        PdfOutlineRoot *outline_root = doc->Outlines();
        PdfOutlineItem *outline_item = new PdfOutlineItem(outline_root);
        outline_item->SetTitle("SJIS Encoding");
        outline_item->SetOpened(true);

        for (unsigned int a = 0x81; a <= 0x9f; a++)
            if (a != 0x85 && a != 0x86)
                DrawPage(doc, outline_item, a);

        for (unsigned int a = 0xe0; a <= 0xee; a++) 
            if (a != 0xeb && a != 0xec)
                DrawPage(doc, outline_item, a);
        
        for (unsigned int a = 0xfa; a <= 0xfc; a++) 
            DrawPage(doc, outline_item, a);
        
        /* Save the document to the file. */
        doc->WriteToFile("EncodingListMb_SJIS.pdf");
...
void DrawPage(PdfDoc *doc, PdfOutlineItem *outline_item, unsigned int from)
{
    ...
    PdfFont* f = doc->FontMgr()->GetFont("gothic");
