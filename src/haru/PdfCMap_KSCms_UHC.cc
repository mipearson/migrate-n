/*
 * << H a r u --free pdf library >> -- PdfCMap_KSCms_UHC.cc
 *
...
static const pdf_mb_unicode_map1 KSCms_UHC_UNICODE_ARRAY1[] = {
    {0x814A, 0xAC18},
    {0x8157, 0xAC2E},
    {0x8173, 0xAC4C},
    {0x817A, 0xAC55},
...
static const pdf_mb_unicode_map2 KSCms_UHC_UNICODE_ARRAY2[] = {
    {0x00, 0x80, 0x0000},
...
void
PdfCMap_KSCms_UHC_H::Init()
{
    AddCMap(CID_ARRAY1);
    SetCIDSystemInfo("Adobe", "Korea1", 1);
    SetUnicodeArray(KSCms_UHC_UNICODE_ARRAY1, KSCms_UHC_UNICODE_ARRAY2);
}

#define KSC_MS_UHC_LEAD_BYTE(c) \
    ((c >= 0x81 && c <= 0xc8) || (c >= 0xca && c <= 0xfd))
#define KSC_MS_UHC_TRIAL_BYTE(c) \
    ((c >= 0x41 && c <= 0x5a) || \
     (c >= 0x61 && c <= 0x7a) || \
     (c >= 0x81 && c <= 0xfe))
...
void 
PdfCMap_KSCms_UHC_H::ParseText(const char* text, pdf_byte_type* btype)
{
    ...
    for (unsigned int i = 0; i < strlen(text); i++, ptext++, ptype++) {
        if (type == PDF_BYTE_TYPE_LEAD)
            if KSC_MS_UHC_TRIAL_BYTE((unsigned char)*ptext)
            ...
            if KSC_MS_UHC_LEAD_BYTE((unsigned char)*ptext)
