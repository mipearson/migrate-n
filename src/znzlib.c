...
// added Peter Beerli 2010
// using zlib 1.2.5 (it is not present in 1.2.3 on macs)
int znzbuffer(znzFile file, unsigned size)
{
#ifdef  HAVE_GZBUFFER 
...
int znzungetc(int c, znzFile file)
{
  if (file==NULL) { return 0; }
#ifdef HAVE_ZLIB
  if (file->zfptr!=NULL) return gzungetc(c, file->zfptr);
#endif
  return ungetc(c,file->nzfptr);
}
